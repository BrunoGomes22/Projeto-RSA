...
class RelayLink {
    ...
    // target_desire represents the desire for the drone to reach its target, if this value is smaller the drone will
    // try to reach the target with caution, if its larger the drone will try to reach the target position faster with
    // possible overshoot, this is used to improve the network
    double target_desire = 1/100
    double prediction_time = 2
    double acceptance_radius = 5

    RelayLink(def coord_drone_side, def child, def distance) {
    ...
    public str = {
        def RESET_COLOR = "\u001B[0m"
        def BLUE = "\u001B[34m"
        return BLUE + "relay link: {" + RESET_COLOR + "coord: [lat: " + coord_drone_side.lat + ", lon: " +
                coord_drone_side.lon + "]" + ", entity_id: [" + entity_id + "]" + ", entity: [" + entity + "]" + BLUE + "}" + RESET_COLOR
    }

}

class DroneRelayBridge {
    def mission_drone
    def relay_links = []
    def connected_relay_link

    DroneRelayBridge(mission_drone, relay_link) {
    ...
    def update_relay_bridge = { closest_relay_link, networks ->
        ...
        if (!networks.containsKey(relay_links[-1].entity.id) || networks[relay_links[-1].entity.id] == null)
            return
...
class NetworkState {
    ...
    def update_mission_drones = { mission_drones ->
    ...
    def add_new_relay_bridges = { mission_drones ->
        for(drone: mission_drones) {
            if(!this.relay_bridges.containsKey(drone.id)) {
